...
export class ToolRunnerNode extends BaseNode {
    ...
    /** 决定工具调用 */
    async decideToolCall(state, stepDescription) {
        if (this.provider) {
            return this.decideWithProvider(stepDescription);
        }
        const providerConfig = state.artifacts.providerConfig;
    ...
    /** 使用 Provider 决定工具调用 */
    async decideWithProvider(stepDescription) {
    ...
    /** 使用 Fetch 调用 */
    async decideWithFetch(stepDescription, baseURL, model) {
        const tools = buildOpenAIToolsList(this.toolRegistry);
        if (tools.length === 0)
            return { call: null, usage: undefined };
        try {
